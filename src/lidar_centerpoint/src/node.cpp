...
: Node("lidar_center_point", node_options), tf_buffer_(this->get_clock())
{
  ...
  const std::string densification_world_frame_id =
    this->declare_parameter("densification_world_frame_id", "base_link");
  const int densification_num_past_frames =
    this->declare_parameter("densification_num_past_frames", 1);
  const std::string trt_precision = this->declare_parameter("trt_precision", "fp32");
  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr input_pointcloud_msg)
{
  ...
  ofstream of ("/home/mlabszw/autoware_singlenode/evaluate/latency.txt",ios::app);
  ...
  ofstream of1 ("/home/mlabszw/autoware_singlenode/evaluate/input_pointcloud_num.txt",ios::app);
